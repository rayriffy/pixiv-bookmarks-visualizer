...
export async function searchIllusts(searchRequest: SearchRequest): Promise<SearchResult> {
    ...
    const illusts = illustResults.map((result) => {
        const user = usersByIllustId.get(result.illusts.id);
        if (!user) {
            // If no user data is found, create a placeholder user
            // This should not happen in normal circumstances
            console.error(`No user data found for illust ${result.illusts.id}`);
            return null as unknown as ExtendedPixivIllust; // This would be caught later by the filter
        }
    ...
    const validIllusts = illusts.filter(Boolean);

    return {
        illusts: validIllusts,
        count: total,
        paginate: {
            current: page,
