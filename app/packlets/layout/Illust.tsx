...
export const Illust = memo<Props>((props) => {
    ...
    const isBlur = (illust.bookmark_private || illust.tags.some((tag) => tag.name === "R-18")) && blur;

    return (
        <div className="mx-auto relative overflow-hidden">
            <a href={`https://www.pixiv.net/artworks/${illust.id}`} target="_blank" rel="noreferrer noopener noreferer">
            ...
            <div className={clsx("relative overflow-hidden", isBlur ? "blur-xl" : "")}>
                {illust.type === "ugoira" ? (
                    <FrontImage {...props} />
                ) : (
                    <img
                ...
                {slicedImage.map((image, i) => (
                    <img
                        ...
                        className={`absolute h-auto left-0 right-0 mx-auto rounded-lg max-h-[600px] object-cover ${
                            i === 0 ? "-bottom-2 -z-10 w-11/12 shadow-lg" : "-bottom-4 -z-20 w-10/12 shadow-xl"
                        }`}
                    />
                ))}
            </div>
