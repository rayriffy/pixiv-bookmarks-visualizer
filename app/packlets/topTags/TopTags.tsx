...
export const TopTags = memo<Props>(({ tags }) => {
    ...
    const filteredTags = useMemo(() => {
        ...
        return tags.filter(
            (tag) => !includeTagNames.includes(tag.name.original) && !excludeTagNames.includes(tag.name.original),
        );
                ...
                {filteredTags.length === 0 ? (
                    ...
                    filteredTags.map((tag) => (
                        ...
                        >
                            ...
                            {tag.name.translated && (
                                <span className="px-1 text-base-content/60">{tag.name.translated}</span>
                            )}
