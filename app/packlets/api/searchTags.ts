 ...
 */
export async function searchTags(request: TagSearchRequest): Promise<TagSearchResponse> {
    ...
    if (selectedTags.length > 0) {
        ...
        if (tagIllustSets.length > 0 && tagIllustSets[0].size > 0) {
            ...
            filteredIllustIds = Array.from(baseSet).filter((id) => sortedSets.every((set) => set.has(id)));
        }
    ...
    async function getTagDetails(tagIds: number[]): Promise<
        Array<{
            id: number;
            name: string;
    ...
    for (const tag of tagDetails) {
        ...
        if (!uniqueTags.has(tag.name) || tagCountsMap[tag.id] > (uniqueTags.get(tag.name)?.count ?? 0)) {
