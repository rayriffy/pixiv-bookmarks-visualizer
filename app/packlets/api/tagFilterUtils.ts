 ...
 */
export const processTagFilters = async (tags: ProcessedTags, existingFilters: SQL[] = []): Promise<number[] | null> => {
    ...
    if (includeTags.length > 0) {
            ...
            validTagIds.map(async (tagId) => {
                ...
                const results = await batchedQuery(allIllustIds, async (batchIds) => {
                    return db
                        ...
                        .where(and(eq(illustTagsTable.tag_id, tagId), inArray(illustTagsTable.illust_id, batchIds)));
                });
        ...
        if (tagIllustSets.length > 0) {
            ...
            targetIllustIds = Array.from(baseSet).filter((id) => sortedSets.every((set) => set.has(id)));
            ...
            for (const tagId of validTagIds) {
                ...
                await batchedQuery(baseIllustIds, async (batchIllustIds) => {
                    ...
                    const results = await db
                        ...
                        .from(illustTagsTable)
                        .where(
                            and(eq(illustTagsTable.tag_id, tagId), inArray(illustTagsTable.illust_id, batchIllustIds)),
                        );
