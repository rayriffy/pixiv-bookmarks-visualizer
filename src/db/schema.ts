...
export const illustsTable = sqliteTable(
  'illusts',
  {
    ...
    bookmark_private: integer('bookmark_private', {
      mode: 'boolean',
    }).notNull(),
...
export const tagsTable = sqliteTable(
  ...
  table => ({
    ...
    translatedNameIdx: index('tag_translated_name_idx').on(
      table.translated_name
    ),
  })
)

export const illustTagsTable = sqliteTable(
  'illust_tags',
  {
    id: int('id').primaryKey().notNull().unique(),
  ...
  table => ({
    ...
    illustTagIdx: index('illust_tag_composite_idx').on(
      table.illust_id,
      table.tag_id
    ),
  })
...
export const illustUsersTable = sqliteTable(
  'illust_users',
  {
    id: int('id').primaryKey().notNull().unique(),
    illust_id: int('illust_id')
  ...
  table => ({
    ...
    // Add composite index for queries that filter on both columns
    illustUserIdx: index('illust_user_composite_idx').on(
      table.illust_id,
      table.user_id
    ),
  })
