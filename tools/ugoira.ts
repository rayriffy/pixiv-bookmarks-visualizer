...
const ugoiraCacheDirectory = path.join(__dirname, "../.data/ugoiraProxy");
...
const initDb = () => {
    ...
    const client = new Database(DB_FILE_NAME);
        ...
        while (attempt === 0 || failures.length !== 0) {
            ...
            for (let i = 0; i < currentBatch.length; i += BATCH_SIZE) {
                ...
                await Promise.allSettled(
                    batchIds.map((id) =>
                        queue.add(async () => {
                            const targetEncodedFile = path.join(ugoiraCacheDirectory, `${id}.webp`);
                            ...
                            try {
                                ...
                                const fileName = path.basename(metadata.ugoira_metadata.zip_urls.medium);
                                const fetchedZip = await fetch(metadata.ugoira_metadata.zip_urls.medium, {
                                    method: "GET",
                                    headers: {
                                ...
                                const targetUgoiraDirPath = path.join(targetExtractPath, id.toString());
                                ...
                                const inputArgs = metadata.ugoira_metadata.frames.flatMap((frame) => [
                                    "-d",
