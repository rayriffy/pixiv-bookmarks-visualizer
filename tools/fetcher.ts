...
const getExistingIllustIds = async (db: any): Promise<Set<number>> => {
  ...
  const existingIllusts = await db
...
const getBookmarks = async (
  ...
  try {
    ...
    const newBookmarks = extendedBookmarks.filter(
      bookmark => !existingIllustIds.has(bookmark.id)
    )
    ...
    if (newBookmarks.length > 0) {
      ...
      for (let i = 0; i < newBookmarks.length; i += BATCH_SIZE) {
        ...
        console.log(
          `Processing batch ${i / BATCH_SIZE + 1}/${Math.ceil(newBookmarks.length / BATCH_SIZE)}...`
        )
    ...
    while (nextUrl !== null) {
      try {
        ...
        // Filter out already existing bookmarks
        const newPageBookmarks = pageBookmarks.filter(
          bookmark => !existingIllustIds.has(bookmark.id)
        )

        // Calculate duplication rate
        const dupPercentage =
          ((pageBookmarks.length - newPageBookmarks.length) * 100) /
        ...
        if (e.response && e.response.status === 429) {
          console.log(
            `Too many requests. Cooling down for ${attempt} minute(s)...`
          )
    ...
    if (attempt < 5) {
      console.log(
        `Exception caught. Performing attempt #${attempt} in ${attempt} minute(s)...`
      )
      console.error(e)
      await new Promise(resolve => setTimeout(resolve, attempt * 60000))
      return getBookmarks(
...
;(async () => {
  try {
    ...
    console.log(
      `Found ${existingIllustIds.size} existing illustrations in the database`
    )
