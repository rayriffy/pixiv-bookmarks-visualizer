...
import { illustsTable, illustTagsTable, illustUsersTable, tagsTable, usersTable } from '../src/db/schema'
...
const getExistingIllustIds = async (db: any): Promise<Set<number>> => {
  ...
  return new Set(existingIllusts.map((i: any) => i.id))
}
  ...
  for (const illust of illusts) {
    ...
    try {
      ...
      for (const tag of illust.tags) {
        ...
        await db.insert(illustTagsTable).values({
          id: parseInt(relationId.replace(/-/g, '')),
          illust_id: illust.id,
          tag_id: tagId
        }).onConflictDoNothing()
      }
...
const getBookmarks = async (
  ...
  try {
    ...
    let extendedBookmarks = bookmarks.map(bookmark => ({
      ...bookmark,
      bookmark_private: restrict === 'private'
    })) as ExtendedPixivIllust[]

    // Filter out already existing bookmarks
    let newBookmarks = extendedBookmarks.filter(bookmark => !existingIllustIds.has(bookmark.id))
    ...
    while (nextUrl !== null) {
      try {
        ...
        console.log(`Page ${page}: Found ${newPageBookmarks.length} new bookmarks (${dupPercentage.toFixed(2)}% duplication)`)
        ...
        if (newPageBookmarks.length > 0) {
          for (let i = 0; i < newPageBookmarks.length; i += BATCH_SIZE) {
            ...
            console.log(`Processing batch ${i/BATCH_SIZE + 1}/${Math.ceil(newPageBookmarks.length/BATCH_SIZE)}...`)
            await insertIllustrations(db, batch, existingTagMap)
...
;(async () => {
  try {
    ...
    console.error('Error during bookmark fetching:', e?.response?.data ?? e)
  }
})()