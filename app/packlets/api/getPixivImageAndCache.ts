...
const cacheDirectory = path.join(process.cwd(), ".data/pixivProxy");
...
export const getPixivImageAndCache = async (url: string) => {
    ...
    const expectedCachePath = isValidGroup
        ? path.join(
              cacheDirectory,
              Math.floor(Number(pureFileName.split("_")[0]) / dividerGroup).toString(),
    ...
    if (!fs.existsSync(cacheDirectory)) await fs.promises.mkdir(cacheDirectory, { recursive: true });
